...
///
/// Expects fresh vm-mgr NV state. Run the simulation first:
...
async fn main() {
    ...
    match orchestrator.flash_all(vec![EcuTarget {
        ...
        Ok(phase) => {
            ...
            // Now back to v1.0.0 (same secver=1, should work)
            match orchestrator.flash_all(vec![EcuTarget {
                component_id: "os1".into(),
                gateway_id: gw.clone(),
                package: load_suit(&format!("{output_dir}/os1-v1.0.0.suit")),
    ...
    // ================================================================
    // Test 3: Flash v1.2.0 then rollback (don't commit)
    // ================================================================
    ...
    // ================================================================
    // Test 4: Reinstall same version (v1.0.0 → v1.0.0)
    // ================================================================
    ...
    match orchestrator.flash_all(vec![EcuTarget {
        ...
        Ok(phase) => {
            // CRL should be Policy type, already committed
            ...
            if os1.update_type != UpdateType::Policy {
                ...
                // Verify v1.0.0 (secver=1) is now rejected
                match orchestrator.flash_all(vec![EcuTarget {
                    component_id: "os1".into(),
                    gateway_id: gw.clone(),
                    package: load_suit(&format!("{output_dir}/os1-v1.0.0.suit")),
                    ...
                    Err(_) => {
                        // v1.2.0-secver2 (re-signed with secver=2) should still work
                        match orchestrator.flash_all(vec![EcuTarget {
                            component_id: "os1".into(),
                            gateway_id: gw.clone(),
...
fn assert_state(
    ecus: &[sumo_sovd_orchestrator::campaign::EcuStatus],
    comp: &str,
    expected: EcuState,
) {
    let ecu = ecus.iter().find(|e| e.component_id == comp)
        .unwrap_or_else(|| panic!("ECU {comp} not found in results"));
